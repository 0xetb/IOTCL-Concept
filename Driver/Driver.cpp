...
NTSTATUS DriverEntry(PDRIVER_OBJECT DriverObject, PUNICODE_STRING RegistryPath) {
    ...
    status = IoCreateDevice(DriverObject, 0, &name, FILE_DEVICE_UNKNOWN, FILE_DEVICE_SECURE_OPEN, FALSE, &device_obj);
    if (!NT_SUCCESS(status)) {
        return status;
    }
    ...
    for (int i = 0; i <= IRP_MJ_MAXIMUM_FUNCTION; i++) {
        DriverObject->MajorFunction[i] = &unsupported_dispatch;
    }

    DriverObject->MajorFunction[IRP_MJ_CREATE] = &dispatch_handler;
    DriverObject->MajorFunction[IRP_MJ_CLOSE] = &dispatch_handler;
    DriverObject->MajorFunction[IRP_MJ_DEVICE_CONTROL] = &io_controller;
